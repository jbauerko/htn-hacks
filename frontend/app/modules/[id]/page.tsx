...
export default async function ModulePage({ params }: Params) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:py-12">
      <Link
        href="/"
        className="inline-flex items-center gap-2 rounded-full bg-white/70 px-3 py-1.5 text-sm font-medium text-zinc-600 shadow-sm ring-1 ring-zinc-200 hover:bg-white dark:bg-zinc-900/60 dark:text-zinc-300 dark:ring-zinc-800"
      >
        ← All roles
      </Link>

      <header
        className={`relative mt-6 overflow-hidden rounded-3xl ${t.bgGradient} px-8 py-8 text-white shadow-lg sm:px-10 sm:py-10`}
      >
        <div className="absolute -right-4 -top-4 text-[10rem] opacity-25 leading-none select-none">
          {detail.icon}
        </div>
        <div className="relative">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            <Chip>{detail.knowledge_item_count} topics</Chip>
            <Chip>{detail.rule_count} non-negotiable rules</Chip>
            <Chip>{detail.glossary.length} glossary terms</Chip>
          </div>
        </div>
      </header>

      <section className="mt-8">
        <h2 className="mb-3 px-1 text-sm font-bold uppercase tracking-wider text-zinc-500">
          Mini-games
        </h2>
          ...
          {GAMES.map((g) => (
            <li key={g.id}>
              <Link
                ...
                className={`group flex h-full items-center gap-4 rounded-2xl border-2 ${t.border} bg-white p-4 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-zinc-900`}
              >
                <div
      ...
      <section className="mt-10">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <details className="group">
            <summary className="cursor-pointer text-sm font-bold text-zinc-700 dark:text-zinc-200">
